...
from chatxz.rnsd import transport as rust_transport
...
def ensure_rns_ports_free(force=False):
    ...
    if sys.platform == "win32":
        ...
        print("[startup] Stop it with:  pkill -f chatxz.rnsd")
...
class ChatRnsDaemon:
    def __init__(
        ...
        self.host = host
        self.port = port
        self.public_port = int(public_port or 8742)
    ...
    def _on_media_packet(self, peer_hash, data):
        rust_transport.forward_media(peer_hash, data)

    def _on_call_signaling(self, peer_hash, content):
        rust_transport.forward_signaling(peer_hash, content)
    ...
    async def handle_restart(self, request):
        ...
        def _source_restart():
            ...
            if sys.platform == "win32":
                ...
                args = [sys.executable, "-m", "chatxz.rnsd", *sys.argv[1:]]
                env = os.environ.copy()
                env["CHATXZ_ROOT"] = root
                env["PYTHONPATH"] = root
