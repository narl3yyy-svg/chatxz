...
class CallAudioManager:
    ...
    def configure(
        ...
        *,
        ...
        if device_prefs is not None:
            self._device_prefs = dict(device_prefs)
    ...
    def set_device_prefs(self, device_prefs: Optional[dict]) -> None:
        with self._lock:
            self._device_prefs = dict(device_prefs or {})

    def start(self, device_prefs: Optional[dict] = None) -> bool:
        ...
        engine = CallAudioEngine(self._send_audio, device_prefs=prefs)
        ok = False
        try:
            ok = engine.start()
